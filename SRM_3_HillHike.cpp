...
class HillHike
{
  ...
  protected:
    ...
    long long getSubPath(int endHeight, int d, int landmarkIndex, bool reachMax)
    {
      ...
      if (!reachMax && endHeight == maxHeight)
        return 0;
      ...
      if (d == 1)
      {
        if (landmarkIndex + 1 > 0)
        {
          if (landmarks[0] != endHeight)
            return 0;
        }
        if (reachMax && endHeight != maxHeight)
          return 0;
      printf("%d %d %d %d \n", endHeight,d, 1, reachMax);
        return 1;
        //if (endHeight != 0 || sublands
      ...
      else
      {
        for (int i = 0; i < 3; i++)
        {
          if (reachMax)
          {
            count += getSubPath(endHeight+delta[i], d-1, landmarkIndex, true);

            if (endHeight == maxHeight)
            {
              count += getSubPath(endHeight+delta[i], d-1, landmarkIndex, false);
          ...
          if (landmarkIndex + 1 > 0 && landmarks[landmarkIndex] == endHeight)
          {
            if (reachMax)
            {
              count += getSubPath(endHeight+delta[i], d-1, landmarkIndex - 1, true);

              if (endHeight == maxHeight)
              {
                count += getSubPath(endHeight+delta[i], d-1, landmarkIndex - 1, false);
