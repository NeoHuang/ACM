...
class HillHike
{
  public:
...

    {
      //Non Recursion
      //
      long long cache1[2][52][51];
      long long cache2[2][52][51];
      int c = sizeof(cache1);
      ...
      long long count = cache1[1][1][landmarks.size() -1];
        printf("%I64d \n", count);
        return count;
...
//My implementation
      ...
      if (distance < maxHeight * 2 )
        return 0;
      else
      {

        int count = getSubPath( 0, distance, landmarks.size() - 1, true);
        printf("%d \n", count);
        return count;
  ...
  protected:
    ...
    long long getSubPath(int endHeight, int d, int landmarkIndex, bool reachMax)

    {
      ...
      if (d == 0)
      {
        if (endHeight != 0 || landmarkIndex > 0 || reachMax != false)
          return 0;
        else
          return 1;
      ...
      if (endHeight == maxHeight)
      {
        ...
        if (landmarkIndex < landmarks.size() && endHeight == landmarks[landmarkIndex])
        {
            count = (getSubPath(endHeight -1, d -1, landmarkIndex -1, true) +
                getSubPath(endHeight - 1, d - 1, landmarkIndex - 1, false) +
                getSubPath(endHeight , d - 1, landmarkIndex - 1, true));
        }
        else
        {
            count = (getSubPath(endHeight -1, d -1, landmarkIndex , true) +
                getSubPath(endHeight - 1, d - 1, landmarkIndex , false) +
                getSubPath(endHeight , d - 1, landmarkIndex , true));
        }
      }
      else
      {
        if (landmarkIndex < landmarks.size() && endHeight == landmarks[landmarkIndex])
        {
            count = (getSubPath(endHeight + 1, d -1, landmarkIndex -1, reachMax) +
                getSubPath(endHeight - 1, d - 1, landmarkIndex - 1, reachMax) +
                getSubPath(endHeight , d - 1, landmarkIndex - 1, reachMax));

        }
        else
        {
            count = (getSubPath(endHeight + 1, d -1, landmarkIndex , reachMax) +
                getSubPath(endHeight - 1, d - 1, landmarkIndex , reachMax) +
                getSubPath(endHeight , d - 1, landmarkIndex , reachMax));
        }
      }
      printf( "%d %d %d %d %d\n", endHeight, d, landmarkIndex, reachMax, count);
      return count;
...
int main(int argc, char** argv)
{
  HillHike hill;
  vector<int> landmarks;
  vector<int> landmarks1;
  landmarks.push_back(4);
  landmarks.push_back(5);
  landmarks.push_back(8);
  landmarks.push_back(5);
  landmarks.push_back(6);
  hill.numPaths(5,2 ,landmarks1); 
  hill.numPaths(38,11 ,landmarks); 
